...
interface RestaurantOrdersTabProps {
    acceptedOrderState: orderType[];
    setAccepted: React.Dispatch<React.SetStateAction<orderType[]>>;
    pendingOrderState: orderType[];
    setPendingOrders: React.Dispatch<React.SetStateAction<orderType[]>>;
}
...
const RestaurantOrdersTab: React.FC<RestaurantOrdersTabProps> = ({
    ...
    const { status: pendingStatus } = useQuery({
    ...
    const { status: acceptedStatus } = useQuery({
    ...
    const { mutate: updateStatus } = useMutation({
        mutationFn: ({ orderId, status, code }: { orderId: number; status: string; code?: string }) =>
    ...
    const handleUpdateStatus = (orderId: number, status: string, code?: string) => {
    ...
    return (
        <div className="space-y-8">
            ...
            <motion.div
                ...
                className="space-y-6"
            >
                <h3 className="text-2xl font-bold text-secondary-900">Pending Orders</h3>
            ...
            <motion.div
                ...
                className="space-y-6"
            >
                <h3 className="text-2xl font-bold text-secondary-900">Accepted Orders</h3>
