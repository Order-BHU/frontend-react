...
import { FiPlus, FiEdit2, FiTrash, FiCheck } from "react-icons/fi";
import { useQuery, useMutation } from "@tanstack/react-query";
import { getCategories, getMenuItems, addMenu, editMenu, deleteMenuItem, updateItemAvailability } from "@/api/restaurant";
import { menu, menuItem, category } from "@/interfaces/restaurantType";
import { useToast } from "@/hooks/use-toast";
import ButtonLoader from "@/components/buttonLoader";

interface RestaurantMenuTabProps {
    menuItemArrayState: menuItem[];
    setmenuItemArray: React.Dispatch<React.SetStateAction<menuItem[]>>;
}
...
const RestaurantMenuTab: React.FC<RestaurantMenuTabProps> = ({
    ...
    // Fetch menu items
    const { data: menuData, status: menuStatus, refetch: refetchMenu } = useQuery({
        queryFn: () => getMenuItems("1"), // Assuming restaurant ID is 1
        queryKey: ["menuItems", "1"],
        onSuccess: (data: menu) => {
            const allItems = data.menus.flatMap(category => category.menus);
    ...
    const { mutate: addMenuItem, isPending: isAdding } = useMutation({
        ...
        onSuccess: (data) => {
            ...
            setNewMenuItem({ name: "", description: "", price: "", category_id: "", image: null });
            refetchMenu();
        },
        onError: (error: any) => {
            toast({
    ...
    const handleAvailabilityToggle = (menuId: number, currentAvailability: "1" | "0") => {
    ...
    if (menuStatus === "pending" || categoryStatus === "pending") {
        return (
    ...
    return (
        <div className="space-y-8">
            ...
            >
                <h3 className="text-2xl font-bold text-secondary-900">Menu Management</h3>
                <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
                    ...
                    <DialogContent className="max-w-md">
                        ...
                        <div className="space-y-4">
                            ...
                            <Button onClick={handleAddMenuItem} disabled={isAdding} className="w-full">
                    ...
                    <div key={item.id} className="bg-white rounded-lg border p-4 space-y-3">
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex gap-2">
                                <Button
                                    variant={item.is_available === "1" ? "destructive" : "default"}
                                    size="sm"
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <Input
                                    ...
                                    type="file"
                                    accept="image/*"
                                    onChange={(e) => setEditingItem({ ...editingItem, image: e.target.files?.[0] || editingItem.image })}
                                />
                            </div>
                            <Button onClick={handleEditMenuItem} disabled={isEditing} className="w-full">
